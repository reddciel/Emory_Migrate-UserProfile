...
.DESCRIPTION
    This tool discovers the location of and migrates
    user data and registry settings to the user's
    local profile.

    Profiles are searched for in the following order:
        FS-Logix: User already migrated; exit.
        Citrix UPM: Migrate Citrix UPM profile and exit.
        Autometrix: Migrate Autometrix profile and exit.
...
.PARAMETER Force
    Forces a migration attempt even if FS-Logix
    is detected on initial check.
...
$FSLogixRegKey = 'HKCU:\Software\Microsoft\Office\15.0\Outlook\Profiles'
